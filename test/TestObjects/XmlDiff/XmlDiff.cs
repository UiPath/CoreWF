...
namespace Test.Common.TestObjects.XmlDiff
{
    public class XmlDiff
    {
        ...
        public bool Compare(XmlReader source, XmlReader target, XmlDiffAdvancedOptions advOptions)
        {
            ...
            {
                expr = nav.Compile(advOptions.IgnoreChildOrderExpr);
                if (advOptions.Context != null)
                {
                    expr.SetContext(advOptions.Context);
                }
            ...
            {
                expr = nav.Compile(advOptions.IgnoreNodesExpr);
                if (advOptions.Context != null)
                {
                    expr.SetContext(advOptions.Context);
                }
            ...
            {
                expr = nav.Compile(advOptions.IgnoreValuesExpr);
                if (advOptions.Context != null)
                {
                    expr.SetContext(advOptions.Context);
                }
        ...
        // overload with XPath to ignore child order
        // ignores the order of the children of the nodes that get selected by the provided path expression
        //public bool Compare(XmlReader source, XmlReader target, string xpathExpr, XmlNamespaceManager mngr)
        //{
        //    InitFiles();
        //    this._SourceDoc.Load(source);
        //    this._TargetDoc.Load(target);
        //    this._SourceDoc.SortChildren(xpathExpr, mngr);
        //    this._TargetDoc.SortChildren(xpathExpr, mngr);
        //    return Diff();
        //}
        ...
        {
            bool flag = false;
            _Writer = XmlWriter.Create(new StringWriter(_Output));
        ...
        {
            //FileStream file;
            //file = PartialTrustFileStream.CreateFileStream(fileName, FileMode.Create, FileAccess.ReadWrite);
            //StreamWriter writer = PartialTrustStreamWriter.CreateStreamWriter(file);
        ...
        {
            //StreamWriter writer = PartialTrustStreamWriter.CreateStreamWriter(stream);
            //writer.Write("<?xml-stylesheet type='text/xsl' href='diff.xsl'?>");
            //writer.Write(ToXml());
            //writer.Close();
        }
