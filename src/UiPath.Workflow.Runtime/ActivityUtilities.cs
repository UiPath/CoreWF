...
internal static class ActivityUtilities
{
    ...
    internal static void ValidateRootInputs(Activity rootActivity, IDictionary<string, object> inputs)
    {
        ...
        if (inputs != null)
        {
                ...
                foreach (RuntimeArgument argument in arguments)
                {
                    if (argument.Name == key)
                    {
                        ...
                        if (inputs.TryGetValue(key, out object inputArgumentValue))
                        {
                            ...
                            {
                                Add(ref validationErrors, new ValidationError(SR.InputParametersTypeMismatch(argument.Type, argument.Name), rootActivity));
                            }
                        }
            ...
            {
                Add(ref validationErrors, new ValidationError(SR.UnusedInputArguments(unusedArguments.AsCommaSeparatedValues()), rootActivity));
            }
        }
    ...
    internal static void ValidateEvaluationOrder(IList<RuntimeArgument> runtimeArguments, Activity referenceActivity, ref IList<ValidationError> validationErrors)
    {
        for (int i = 0; i < runtimeArguments.Count - 1; i++)
        {
            ...
            {
                Add(ref validationErrors, new ValidationError(SR.DuplicateEvaluationOrderValues(referenceActivity.DisplayName, argument.BoundArgument.EvaluationOrder), false, argument.Name, referenceActivity));
            }
        }
    }

    internal static string GenerateExceptionString(IList<ValidationError> validationErrors, ExceptionReason reason)
    {
        ...
        if (constraints != null)
        {
            ...
            for (int constraintIndex = 0; constraintIndex < constraints.Count; constraintIndex++)
            {
                ...
                catch (Exception e)
                {
                    ...
                    Add(ref validationErrors, constraintExceptionValidationError);
                }
    ...
    {
        FinishCachingSubtree(root, parentChain, options);
    ...
    private static void ProcessActivity(ChildActivity childActivity, ref ChildActivity nextActivity, ref Stack<ChildActivity> activitiesRemaining, ActivityCallStack parentChain, ref IList<ValidationError> validationErrors, ProcessActivityTreeOptions options, ProcessActivityCallback callback)
    {
        ...
        if (!activity.HasStartedCachingMetadata)
        {
            ...
            ValidateArguments(activity, activity.Parent == null, ref tempValidationErrors);
        ...
        {
            RunConstraints(childActivity, parentChain, constraints, options, false, ref validationErrors);
        }
    }
        ...
        if (!ShouldShortcut(activity, options))
        {
            lock (activity.ThisLock)
            {
                if (!ShouldShortcut(activity, options))
                {
                    ...
                    // fully cached.
                    if (!HasErrors(validationErrors) && options.IsRuntimeReadyOptions)
                    {
