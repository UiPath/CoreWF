...
using Microsoft.CoreWf.Statements.Tracking;
...
namespace Microsoft.CoreWf.Statements
{
    ...
    internal sealed class InternalState : NativeActivity<string>
    {
        ...
        private Collection<InternalTransition> _internalTransitions;

        // number of running triggers
        private Variable<int> _currentRunningTriggers;
        private Variable<bool> _isExiting;

        // This bookmark is used to evaluate condition of a transition of this state. 
        private Variable<Bookmark> _evaluateConditionBookmark;

        // Callback which is called when Entry is completed.
        private CompletionCallback _onEntryComplete;

        // Callback which is called when Trigger is completed.
        private CompletionCallback _onTriggerComplete;
        ...
        private CompletionCallback _onExitComplete;

        // Callback which is used to start to evaluate Condition of a transition of this state.
        private BookmarkCallback _evaluateConditionCallback;

        private Dictionary<Activity, InternalTransition> _triggerInternalTransitionMapping = new Dictionary<Activity, InternalTransition>();
        ...
        public InternalState(State state)
        {
            ...
            _onEntryComplete = new CompletionCallback(this.OnEntryComplete);
            _onTriggerComplete = new CompletionCallback(this.OnTriggerComplete);
            _onConditionComplete = new CompletionCallback<bool>(this.OnConditionComplete);
            _onExitComplete = new CompletionCallback(this.OnExitComplete);

            _evaluateConditionCallback = new BookmarkCallback(this.StartEvaluateCondition);
        ...
        public string StateMachineName
        {
            get
            {
                return _state.StateMachineName;
            }
        }
        ...
        private void OnTriggerComplete(NativeActivityContext context, ActivityInstance completedInstance)
        {
            ...
            else if (completedInstance.State == ActivityInstanceState.Closed)
            {
                ...
                eventManager.RegisterCompletedEvent(
                    new TriggerCompletedEvent { Bookmark = _evaluateConditionBookmark.Get(context), TriggedId = internalTransition.InternalTransitionIndex },
        ...
        private void ScheduleExit(NativeActivityContext context)
        {
            if (this.Exit != null)
            {
                context.ScheduleActivity(this.Exit, _onExitComplete);
            }
            else
            {
        ...
        {
            return _internalTransitions[triggerIndex];
        }

        private Activity<bool> GetCondition(int triggerIndex, int conditionIndex)
        {
            return _internalTransitions[triggerIndex].TransitionDataList[conditionIndex].Condition;
        }

        private string GetTo(int triggerIndex, int conditionIndex = 0)
        {
            return _internalTransitions[triggerIndex].TransitionDataList[conditionIndex].To.StateId;
        }
