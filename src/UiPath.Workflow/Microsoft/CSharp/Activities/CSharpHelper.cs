    ...
    class CSharpHelper
    {
        ...
        static HostedCompilerWrapper GetCachedHostedCompiler(HashSet<Assembly> assemblySet)
        {
            ...
            lock (HostedCompilerCache)
            {
                ...
                hcompilerWrapper = new HostedCompilerWrapper(new CSJustInTimeCompiler(assemblySet));
        ...
        CSharpHelper(string expressionText)
        {
        ...
        public static void GetAllImportReferences(Activity activity, bool isDesignTime, out List<string> namespaces, out List<AssemblyReference> assemblies)
        {
            ...
            VisualBasicSettings rootCSharpSettings = VisualBasic.GetSettings(environment.Root);
            if (rootCSharpSettings != null)
            {
                // We have CSharpSettings
                ExtractNamespacesAndReferences(rootCSharpSettings, namespaceList, assemblyList);
            }
            else
            {
                ...
                if (isDesignTime)
                {
                    // When called via CSharpDesignerHelper, we don't know whether or not 
        ...
        static void ExtractNamespacesAndReferences(VisualBasicSettings cSharpSettings,
            IList<string> namespaces, IList<AssemblyReference> assemblies)
        {
            foreach (VisualBasicImportReference importReference in cSharpSettings.ImportReferences)
            {
        ...
        public LambdaExpression CompileNonGeneric(LocationReferenceEnvironment environment)
        {
            ...
            var scriptAndTypeScope = new CSharpScriptAndTypeScope(environment, referencedAssemblies.ToList());
        ...
        static readonly FindMatch delegateFindLocationReferenceMatchShortcut = new FindMatch(FindLocationReferenceMatchShortcut);
        static readonly FindMatch delegateFindFirstLocationReferenceMatch = new FindMatch(FindFirstLocationReferenceMatch);
        static readonly FindMatch delegateFindAllLocationReferenceMatch = new FindMatch(FindAllLocationReferenceMatch);
        ...
        class CSharpScriptAndTypeScope
        {
            ...
            public CSharpScriptAndTypeScope(LocationReferenceEnvironment environmentProvider, List<Assembly> assemblies)
            {
